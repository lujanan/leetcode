...
func TestConstructorV2(t *testing.T) {
	var fnName = []string{"cursorLeft", "addText", "cursorLeft", "addText", "cursorRight", "addText", "addText", "addText", "deleteText", "addText", "addText", "addText", "deleteText", "cursorLeft", "addText", "addText", "addText", "cursorRight", "cursorRight"}
	var params = []any{14, "soxqep", 7, "rfdrotsyeoy", 19, "htmnn", "tasaznyhpppw", "uufybgurb", 1, "nxg", "f", "qzsojyx", 6, 12, "q", "lojnjwgzmsitqfpvq", "zf", 15, 3}
	var res []any

	var textEdit = ConstructorV2()
	for i, fn := range fnName {
		var pa = params[i]
		switch fn {
		case "addText":
			(&textEdit).AddText(pa.(string))
			res = append(res, "")

		case "deleteText":
			num := (&textEdit).DeleteText(pa.(int))
			res = append(res, num)

		case "cursorLeft":
			str := (&textEdit).CursorLeft(pa.(int))
			res = append(res, str)

		case "cursorRight":
			str := (&textEdit).CursorRight(pa.(int))
			res = append(res, str)
		}
	}
