...
func waysToReachStair(k int) int {
	var jumpMap  = make(map[int]int)
	var jumpFn func(i, j int) int
	jumpFn = func(i, j int) int {
		if i == k {
			return 1
		}
		...
		if i != 0 {
			return jumpFn(i-1, j) + jumpFn(i+int(math.Pow(2, float64(j))), j+1)
		}
		return jumpFn(i+int(math.Pow(2, float64(j))), j+1)
	}
	return jumpFn(1, 0)
}
