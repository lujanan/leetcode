...
func Test_removeSubfolders(t *testing.T) {
	...
	tests := []struct {
		...
		{"t2", args{[]string{"/a", "/a/b/c", "/a/b/d"}}, []string{"/a"}},
		{"t3", args{[]string{"/a/b/c", "/a/b/ca", "/a/b/d"}}, []string{"/a/b/c", "/a/b/ca", "/a/b/d"}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			wantMap := make(map[string]int)
			resMap := make(map[string]int)
			gotRes := removeSubfolders(tt.args.folder)
			for _, v := range tt.wantRes {
				wantMap[v] = 0
			}
			for _, v := range gotRes {
				if _, ok := wantMap[v]; !ok {
					t.Errorf("removeSubfolders() = %v, want %v", gotRes, tt.wantRes)
					break
				}
				resMap[v] = 0
			}
			if len(resMap) != len(wantMap) {
				t.Errorf("removeSubfolders() = %v, want %v", gotRes, tt.wantRes)
			}
		})
	}
