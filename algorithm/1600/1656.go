...
//
//
// Related Topics 设计 数组 哈希表 数据流 👍 106 👎 0
...
func (this *OrderedStream) Insert(idKey int, value string) []string {
	if idKey < 1 || idKey > len(this.Values) {
		return nil
	}
	...
	if idKey == this.Ptr {
		for {
			this.Ptr++
			if this.Ptr > len(this.Values) || len(this.Values[this.Ptr-1]) <= 0 {
				break
			}
		}
		return this.Values[idKey-1 : this.Ptr-1]
	}
	return nil
}
